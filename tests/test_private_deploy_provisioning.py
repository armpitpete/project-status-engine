...
class WindowsCredentialPreparationTests(unittest.TestCase):
    ...
    def test_windows_powershell_preserves_the_empty_passphrase_argument(self):
        self.assertIn("Windows PowerShell 5.1 drops an unquoted empty", self.script)
        self.assertIn("$sshKeygenArguments = @(", self.script)
        self.assertIn("\"-N\", '\"\"'", self.script)
        self.assertIn("Start-Process", self.script)
        self.assertIn("-ArgumentList $sshKeygenArguments", self.script)
        self.assertIn("$keygenProcess.ExitCode", self.script)
        self.assertNotIn('& $sshKeygen.Source -t ed25519 -N ""', self.script)
